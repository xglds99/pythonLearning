...
class Server():
    ...
    def start_p2p_listen(self):
        ...
        while True:
            ...
            Thread(target=self.p2p_send_msg, args=(connection,)).start()
            Thread(target=self.p2p_recv_msg, args=(connection,)).start()
    ...
    def p2p_send_msg(self, connection):
        # ---------------------------------------------
        # TODO： 实现发送消息功能
        ...
        while True:
            data = input()
            print(f'服务器:{0}\self', data)
            data = data.encode()
            connection.send(bytes('{0}'.format(data), "UTF-8"))
        # try:
        #     # 可能报错的语句
        #     a = 1
        # except:
    ...
    def p2p_recv_msg(self, connection):
        # ---------------------------------------------
        ...
        while True:
            ...
            print(f'客户端:{0}'.format(data.decode('UTF-8')))
