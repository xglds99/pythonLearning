...
def recvived(address, port):
    ...
    print("server started transmission！")
    with open('xjtlu1.jpg', 'rb') as f1:
        while True:
            bytes_send = f1.read(SendBufferSize)
            if not bytes_send:
                print('服务端传输完毕')
                udp_socket.sendto('file_download_exit'.encode('utf-8'), c_address)
                break
            udp_socket.sendto(bytes_send, c_address)
    udp_socket.close()
